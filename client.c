...
/*   Updated: 2024/07/31 14:56:21 by elenasurovt      ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void client_handler(int sig)
{
    ...
    {
        ft_putstr_fd("\e[92m > end of message signal recieved from server\n\e[0m", STDOUT_FILENO);
        exit(EXIT_SUCCES);
    }
}

stattic void client_send_message(int server_pid, char *str)
{
    int i;
